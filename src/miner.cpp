...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SYOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SYOMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SYOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("syologic-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("SYOMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running SYOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadSYOMiner() exception");
    } catch (...) {
        LogPrintf("ThreadSYOMiner() exception");
    }

    LogPrintf("ThreadSYOMiner exiting\n");
}
